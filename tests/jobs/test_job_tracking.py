...
from app.jobs_tracking.services.job_tracking_write_service import JobTrackingWriteService

from app.user.services.user_registry_service import UserRegistryService
from ..mockups.mongo_mockups import MockCompanyMongoReadPersist, MockCompanyMongoWritePersist, MockUserMongoPersist
import mongomock
import time
...
@pytest.fixture
def job_write_service(db):
    mock_persist = MockCompanyMongoWritePersist(db)    
    return JobTrackingWriteService(mock_persist)


@pytest.fixture
def job_read_service(db):
    mock_persist = MockCompanyMongoReadPersist(db)    
    return JobTrackingReadService(mock_persist)
...
@pytest.mark.asyncio
async def test_track_and_retrieve_job(user_service, job_write_service):
    # 1. Register
    email = "dummy.user@example.com"
    auth_res = await user_service.register(email)
    ...
    job_res: JobTrackingResponse = await job_write_service.track_new_job(command)
...
@pytest.mark.asyncio
async def test_get_applications(user_service, job_write_service, job_read_service):
    ...
    # 2. Add new job
    job_tracking_response = await job_write_service.track_new_job(track_command)
    assert job_tracking_response is not None
    assert job_tracking_response.code == JobTrackingResponseCode.OK

    
    # 3. Test job retrieval
    get_command = GetTrackedJobsCommand(user_id=user_id, company_name=company)
    company_response = await job_read_service.get_tracked_jobs(get_command)
...
@pytest.mark.asyncio
async def test_track_new_job_missing_params(job_write_service, sample_job):
    """Test validation for missing parameters."""
    # Missing user_id
    cmd1 = TrackNewJobCommand(user_id="", company_name="Acme", tracked_job=sample_job)
    response = await job_write_service.track_new_job(cmd1)
    ...
    response = await job_write_service.track_new_job(cmd2)
    assert response.code == JobTrackingResponseCode.ERROR

    # Missing job_url
    sample_job.job_url = ""
    cmd3 = TrackNewJobCommand(user_id="user1", company_name="Acme", tracked_job=sample_job)
    response = await job_write_service.track_new_job(cmd3)
    assert response.code == JobTrackingResponseCode.ERROR

@pytest.mark.asyncio
async def test_track_new_job_invalid_contact_name(job_write_service, sample_job):
    ...
    response = await job_write_service.track_new_job(command)
...
@pytest.mark.asyncio
async def test_track_existing_job_missing_params(job_write_service, sample_job):
    ...
    response = await job_write_service.track_existing_job(command)
    assert response.code == JobTrackingResponseCode.ERROR

@pytest.mark.asyncio
async def test_track_existing_job_not_found(job_write_service, sample_job):
...
@pytest.mark.asyncio
async def test_add_new_job_for_existing_company(user_service, job_write_service, job_read_service):
    ...
    res1 = await job_write_service.track_new_job(cmd1)
    ...
    res2 = await job_write_service.track_new_job(cmd2)
    ...
    company_res = await job_read_service.get_tracked_jobs(get_cmd)
...
@pytest.mark.asyncio
async def test_add_new_job_for_new_company(user_service, job_write_service, job_read_service):
    ...
    res = await job_write_service.track_new_job(track_cmd)
...
@pytest.mark.asyncio
async def test_update_existing_job(user_service, job_write_service, job_read_service):
...
@pytest.mark.asyncio
async def test_update_job_timestamp_changes(user_service, job_write_service):
...
@pytest.mark.asyncio
async def test_delete_tracked_jobs(user_service, job_write_service, job_read_service):
    ...
    await job_write_service.track_new_job(track_cmd)
    
    # 3. Verify it exists
    get_cmd = GetTrackedJobsCommand(user_id=user_id, company_name=company_name)
    res = await job_read_service.get_tracked_jobs(get_cmd)
    assert res.code == JobTrackingResponseCode.OK
    ...
    success = await job_write_service.delete_tracked_jobs(del_cmd)
    assert success is True
    
    # 5. Verify it is gone
    res_after = await job_read_service.get_tracked_jobs(get_cmd)
