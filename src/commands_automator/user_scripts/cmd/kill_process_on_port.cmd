...
for /f "tokens=5" %%i in ('netstat -ano -p TCP ^| findstr /R /C:":%PORT% .*LISTENING"') do (
    ...
    if not "!PID!"=="" (
        ...
        for /f "tokens=1 delims=," %%j in ('tasklist /FI "PID eq !PID!" /FO CSV /NH') do (
            set "PROCESS_NAME=%%~j"
        )
        set "PROCESS_NAME=!PROCESS_NAME:"=!"
        if not defined PROCESS_NAME set "PROCESS_NAME=(unknown)"
        echo Process name: !PROCESS_NAME!            set PROCESS_NAME=%%j
            set PROCESS_NAME=!PROCESS_NAME:"=!
        ...
        if !errorlevel! equ 0 (
            echo Successfully killed process !PID! (!PROCESS_NAME!)
        ) else (
            echo Failed to kill process !PID!. You might need administrator privileges.
            exit /b 1
        )
