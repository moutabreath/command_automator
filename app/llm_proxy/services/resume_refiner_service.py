...
class ResumeRefinerService:
    def __init__(self, resume_chat: Chat, gemini_client_wrapper: GeminiClientWrapper):
        self.resume_saver_service: ResumeSaverService = ResumeSaverService()
        self.resume_chat: Chat = resume_chat
        self.gemini_agent: GeminiClientWrapper = gemini_client_wrapper
    ...
    async def refine_resume(self, tool_result: str, output_file_path: str) -> MCPResponse:
        try:
            ...
            if not isinstance(resume_data_dict, dict):
                ...
                return MCPResponse(error_message=error_msg, code=MCPResponseCode.ERROR_WITH_TOOL_RESPONSE)
    ...
    async def get_refined_resume(self, resume_data_dict: dict) -> MCPResponse:
        ...
        if resume_response.code == LLMResponseCode.OK:
            return MCPResponse(result_text=resume_response.text, code=MCPResponseCode.OK)
        if resume_response.code == LLMResponseCode.MODEL_OVERLOADED:
            return MCPResponse(error_message=resume_response.text,code= MCPResponseCode.ERROR_MODEL_OVERLOADED)
        return MCPResponse(error_message= resume_response.text, code=MCPResponseCode.ERROR_COMMUNICATING_WITH_LLM)
    ...
    async def get_cover_letter(self, resume_data_dict: dict) -> MCPResponse:
        ...
        if cover_letter_guidelines:
            ...
            if cover_letter_response.code == LLMResponseCode.OK:
                return MCPResponse(result_text=cover_letter_response.text, code=MCPResponseCode.OK)
            if cover_letter_response.code == LLMResponseCode.MODEL_OVERLOADED:
                return MCPResponse(error_message=cover_letter_response.text, code=MCPResponseCode.ERROR_MODEL_OVERLOADED)
            return MCPResponse(error_message=cover_letter_response.text,code=MCPResponseCode.ERROR_COMMUNICATING_WITH_LLM)
        # No cover letter guidelines provided - not an error
        return MCPResponse(result_text="No cover letter guidelines provided", code=MCPResponseCode.OK)