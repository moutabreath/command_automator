...
from ..services.domain.commands import ExtractJobInfoCommand

def extract_job_title_and_company(extract_job_info_command: ExtractJobInfoCommand):
    
    logging.info(f"start with {extract_job_info_command.url}")
    url =extract_job_info_command.url
    headers = {
